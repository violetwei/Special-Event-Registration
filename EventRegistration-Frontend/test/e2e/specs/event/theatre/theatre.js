...
module.exports = {
  'Test Create Theatre': function (client) {
    ...
    client
      ...
      .setValue(config.id.dateInput, `${event.date.year}-${event.date.day}-${event.date.month}`)
      .pause(config.time.pause)
      ...
      //.assert.containsText(`#${id_name}-date`, `${event.date.year}-${event.date.month}-${event.date.day}`)
      .assert.containsText(`#${id_name}-starttime`, event.startTime)
      .assert.containsText(`#${id_name}-endtime`, event.endTime)
      .end();
  },
